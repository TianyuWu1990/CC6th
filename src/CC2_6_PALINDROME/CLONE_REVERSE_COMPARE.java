...
public class CLONE_REVERSE_COMPARE {
	...
	public static LinkedListNode reverseAndClone(LinkedListNode node) {
		...
		while (node != null) {
			LinkedListNode n = new LinkedListNode(node.data); // Clone, not using the original list
			n.next = head;
			head = n;
			node = node.next;
		}
	...
	public static boolean isEqual(LinkedListNode one, LinkedListNode two) {
		...
		return one == null && two == null;// length equal and data equal
	}
